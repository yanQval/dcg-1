...
class DeepCoordinationGraphBFMAC(BasicMAC):
    ...
    def __init__(self, scheme, groups, args):
        ...
        self.bf_lib = load_c_lib('./src/utils/brute_force.cpp')
    ...
    def greedy(self, f_i, f_ij, available_actions=None):
        ...
        _f = np.array(copy.deepcopy(f_i).cpu()).astype(ctypes.c_double)
        _g = np.array(copy.deepcopy(f_ij).cpu()).astype(ctypes.c_double)
        _edges_from = np.array(copy.deepcopy(self.edges_from)).astype(ctypes.c_double)
        _edges_to = np.array(copy.deepcopy(self.edges_to)).astype(ctypes.c_double)

        _best_actions = np.zeros((f_i.shape[0], f_i.shape[1], 1)).astype(ctypes.c_double)

        self.bf_lib.solve(c_ptr(_f), c_ptr(_g), c_ptr(_edges_from), c_ptr(_edges_to), c_ptr(_best_actions), f_i.shape[0], self.n_agents, self.n_actions, len(self.edges_from))

        best_actions = th.tensor(copy.deepcopy(_best_actions)).to(f_i.device)

        best_actions = best_actions.int()
